...
const { transport, contentEmail }= require('../mail');
...
const Mutations = {
  ...
  async requestReset(parent, args, ctx, info) {
    ...
    const mailRes = await transport.sendMail({
      from: 'noreply@winesupplyshop.com',
      to: user.email,
      subject: 'Your Password Reset Token',
      html: contentEmail(`Your Password Reset Token is here!
      \n\n
      <a href="${process.env
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error(`The user doesn't own the item`);
    }
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    const user = await ctx.db.query.user(
      ...
      cart {
        id
        quantity
        product { name price id description image }
      }}`
    );
